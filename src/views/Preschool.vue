<template>
  <div class="preschool-page">
    ...
    <template v-if="!shouldShowLoading">
      ...
      <TabBar
        v-model="activeTab"
        :tab-items="processedTabItems as any"
...
const {
  ...
  dynamicSidebarItems,
  handleContentClick,
} = useAutoPageEducationForPage('Preschool.vue');

// ============================================================================
// 处理TabItems - 添加图标逻辑
// ============================================================================
// ============================================================================
// 处理TabItems - 修改为使用不同的图标路径
// ============================================================================
const processedTabItems = computed(() => {
  // 检查localStorage中的selectedStage是否为preschool
  const selectedStage = localStorage.getItem('selectedStage');
  const isPreschoolStage = selectedStage === 'preschool';
  
  // 复制原始tabItems数组，避免直接修改
  const processedItems = [...originalTabItems.value];
  
  // 如果是preschool阶段，则为不同的标签分配不同的图标
  if (isPreschoolStage && processedItems.length > 0) {
    // 定义图标路径数组，按顺序对应不同的标签
    const iconPaths = [
      '/src/assets/images/home/icon_class.png',
      '/src/assets/images/home/icon_cartoon.png',
      '/src/assets/images/home/icon_science.png',
      '/src/assets/images/home/icon_connect.png'
    ];
    
    // 为每个标签项分配对应的图标
    return processedItems.map((item, index) => ({
      ...item,
      // 使用模运算确保即使标签数量超过图标数量也能正确显示
      icon: iconPaths[index % iconPaths.length]
    }));
  }
